...
WHERE safe_numeric_cast(value) IS NOT NULL;
    ...
    def extract_float_metadata_from_csv(self, csv_path: str) -> List[Dict]:
        ...
        try:
            ...
            # Determine file type for debugging
            file_type = "BGC" if "BGC" in csv_path or any(col in df.columns for col in ['BBP700', 'CDOM', 'CHLA']) else "Non-BCG"
        ...
        for csv_file in csv_files:
            ...
            if metadata_list:
                for metadata in metadata_list:
                    ...
                    # Create unique identifier for this specific reading
                    ...
                    # Skip if we've already processed this specific reading
                    if reading_id in processed_floats:
                        logger.debug(f"Reading {reading_id} already processed, skipping")
                        continue
    ...
    def verify_table_and_data(self) -> bool:
        ...
        """
        try:
            # Check float_metadata2 table (float-level data with metadata)
            metadata_response = self.client.table('float_metadata2').select('float_id', count='exact').execute()
            metadata_count = metadata_response.count if hasattr(metadata_response, 'count') else len(metadata_response.data)
            logger.info(f"✓ Verification: float_metadata2 table contains {metadata_count} unique floats")
            
            # Check float_parameters_reading2 table (parameter readings with daily locations)
            params_response = self.client.table('float_parameters_reading2').select('id', count='exact').execute()
            params_count = params_response.count if hasattr(params_response, 'count') else len(params_response.data)
            logger.info(f"✓ Verification: float_parameters_reading2 table contains {params_count} parameter records")
