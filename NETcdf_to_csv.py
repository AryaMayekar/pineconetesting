...
def create_final_csv_summaries():
    ...
    for (year, month_name), month_files in files_by_month.items():
        ...
        # Add precise depth calculation from pressure
        if 'PRES' in df.columns:
            print("    Adding precise depth calculation...")
            # Convert pressure (dbar) to depth (meters) using oceanographic approximation
            # For most oceanographic purposes: depth ≈ pressure × 1.019716 (accounting for gravity variation)
            # But 1 dbar ≈ 1 meter is accurate enough for most analyses
            df['DEPTH_m'] = df['PRES'] * 1.0  # Using simple 1:1 ratio (dbar to meters)
            
            # Clean up invalid values
            df.loc[df['DEPTH_m'] < 0, 'DEPTH_m'] = np.nan
            df.loc[df['DEPTH_m'] > 12000, 'DEPTH_m'] = np.nan  # Deepest ocean is ~11km
            
            # Round to 1 decimal place for cleaner output
            df['DEPTH_m'] = df['DEPTH_m'].round(1)
            
            print(f"      Added DEPTH_m column (pressure-based depth in meters)")
        ...
        for name, group in grouped:
            ...
            for col in df.columns:
                if col in ['PLATFORM_NUMBER', 'n_levels', 'DATE', 'row_count', 'JULD', 'JULD_LOCATION']:
                    continue  # Skip JULD and JULD_LOCATION since we process them separately
                ...
                try:
                    ...
                    if df[col].dtype in ['object', 'string']:
                        ...
                        if len(numeric_data) > 0:
                            if col in ['PRES', 'PRES_ADJUSTED', 'DEPTH_m']:
                                # Calculate min/max/mean for pressure and depth columns
